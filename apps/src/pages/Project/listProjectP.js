...
class ListProjectP extends Component{
  ...
  fetchData = async (props)=>{
      fetch("http://mppk-app.herokuapp.com/getDataProjectByIdPegawai/"+this.state.iduser)
      .then(res=>res.json())
      .then(data=>{
        try{
          console.log(data)
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <FlatList
                 renderItem={({item})=>(
                    ...
                    <TouchableOpacity onPress={()=>navigation.navigate('DetailProject',{id:item.project._id,idClient:item.client._id,idp:item.user._id,status:item.project.aktif})}>
                    <View 
                    style={{
                        flexDirection:'row' ,
                        borderBottomWidth:0,
          ...
          <Overlay isVisible={this.state.modalVisible} 
            ...
            <TouchableOpacity style={styles.btnoverlay} onPress={()=>{navigation.navigate('AccountP'),this.setState({modalVisible:false})}}>
...
const styles=StyleSheet.create({
  ...
  modalView: {
    marginLeft:'40%',marginBottom:hp('80%'),
    width:200,
    height:100,
    backgroundColor: "white",
    borderRadius: 20,
...
subtext: {
  ...
  //fontWeight: "bold",
  textAlign:'left',
  marginVertical:0,marginLeft:5,width:wp('60%')
},
});
