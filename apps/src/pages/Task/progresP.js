...
class ProgressP extends Component{
  ...
  fetchDataTask= async (props)=>{
    fetch("http://mppk-app.herokuapp.com/getDataUserTask2/"+this.state.idproject+"/"+this.state.idpegawai)
    .then(res=>res.json())
    .then(data=>{
      console.log(data);
  ...
  onPress(status){
    if(this.state.status== 'yes' && status!='done'){
                              ...
                              <Text style={{fontSize:16, paddingLeft:10,fontWeight:'bold'}}>Kategori Pekerjaan: {item.kategori_pekerjaan}</Text>
                                {
                                  this.state.listTask.map((l)=>(
                                    <View >
                                         { l.module[0].kategori_pekerjaan === item.kategori_pekerjaan ? (
                                          <View style={{}}>
                                  
                                            <TouchableOpacity onPress={()=>{this.onPress(l.status);this.setState({idTask:l.idtask})}}
                                                    style={{
                                                    ...
                                                    <View style={{width:'100%',}}>
                                                            <View style={{flexDirection:'row'}}>
                                                              ...
                                                              <Text style={styles.subtext}>{l.task[0].nama_task}</Text>
                                                            </View>
                                                            <View style={{flexDirection:'row'}}>
                                                              <Text style={styles.text}>Spesifikasi</Text>
                                                              <Text style={styles.titikdua}>:</Text>
                                                              <Text style={styles.subtext}>{l.task[0].spesifikasi} </Text>
                                                            </View>
                                                            <View style={{flexDirection:'row'}}>
                                                              <Text style={styles.text}>Jadwal</Text>
                                                              <Text style={styles.titikdua}>:</Text>
                                                              <Text style={styles.subtext}>{l.task[0].tglMulai} - {l.task[0].tglSelesai}</Text>
                                                            </View>
                                                            <View style={{flexDirection:'row'}}>
