...
class DetailAlat extends Component{
  ...
  deleteData = async (props) =>{
    ...
    fetch('http://mppk-app.herokuapp.com/deleteAlat/'+ this.state.idalat)
    .then(res=>res.json())
    .then(data=>{
        if(!data.error){
      ...
      <View style={{flex:1}}>
          ...
          <ScrollView>
              ...
              <View style={{
                    ...
                    <TouchableOpacity onPress={() => {navigation.navigate('EditAlat',{idalat:this.state.idalat,id:this.state.idproject})}} style={styles.btnupdate}>
